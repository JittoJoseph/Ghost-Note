...
linksRouter.post("/", authMiddleware, zValidator("json", createLinkSchema), async (c) => {
  ...
  try {
    ...
    const url = c.env.FRONTEND_URL ? `${c.env.FRONTEND_URL}/l/${slug}` : `/l/${slug}`;
...
linksRouter.post("/:slug/submit", zValidator("json", submitSchema), async (c) => {
  const slug = c.req.param("slug");
  ...
  try {
    ...
    if (result.length === 0) {
      return c.json({ error: "Link is invalid or has already been used" }, 403);
    }
